...
export async function onRequestPost(context) {
  ...
  try {
    user = await context.env.DB.prepare(
      'SELECT username FROM users WHERE username = ? AND email = ? LIMIT 1',
    ).bind(username, email).first();
  } catch {
    return json({ error: '账号数据库暂时不可用，请稍后重试。' }, 503);
  }
  if (!user) {
    return json({ error: '账号与邮箱不匹配。' }, 404);
  }
  ...
  try {
    emailRequest = await context.env.DB.prepare(
      `SELECT id, code_hash, code_salt, attempts, expires_at
       ...
       WHERE email = ? AND purpose = 'password-reset' AND consumed_at IS NULL
  ...
  try {
    const statements = await context.env.DB.batch([
      context.env.DB.prepare(
        `UPDATE users
         SET password_hash = ?, password_salt = ?, email_verified_at = COALESCE(email_verified_at, CURRENT_TIMESTAMP)
         WHERE username = ? AND email = ?`,
      ).bind(passwordData.hash, passwordData.salt, username, email),
      context.env.DB.prepare(
    ...
    return json({ error: '密码更新失败，请稍后重试。' }, 503);
  }

  let cookie;
  try {
    cookie = await createSessionCookie(user.username, context.env.SESSION_SECRET);
  } catch {
    return json({ error: '密码已重置，请返回登录页登录。' }, 503);
  }
